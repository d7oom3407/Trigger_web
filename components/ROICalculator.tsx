...
const ROICalculator: React.FC<Props> = ({ t, lang, setView }) => {
  ...
  return (
    <section id="pricing" className="py-24 px-6 bg-white dark:bg-black transition-colors">
      <div className="max-w-6xl mx-auto glass-card rounded-[48px] p-8 md:p-16 relative overflow-hidden border-violet-500/20 shadow-[0_0_80px_rgba(139,92,246,0.1)]">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 relative z-10">
          <div className="space-y-10">
            <div className="space-y-4">
              <div className="flex justify-between items-end">
                ...
                <span className="text-3xl font-black text-violet-600 dark:text-violet-400">{employees}</span>
              </div>
              <input 
                type="range" min="1" max="100" step="1" value={employees} 
                onChange={(e) => setEmployees(Number(e.target.value))}
                className="w-full h-2 bg-slate-200 dark:bg-white/10 rounded-lg appearance-none cursor-pointer accent-violet-600"
              />
            </div>
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-end">
                ...
                <span className="text-3xl font-black text-violet-600 dark:text-violet-400">{hoursPerDay}h</span>
              </div>
              <input 
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-end">
                ...
                <span className="text-3xl font-black text-violet-600 dark:text-violet-400">${hourlyRate}</span>
              </div>
              <input 
          ...
          <div className="grid grid-cols-1 gap-6">
            <div className="bg-gradient-to-br from-violet-600 to-indigo-700 rounded-3xl p-10 flex flex-col justify-between shadow-2xl shadow-violet-900/40 border border-white/10 group overflow-hidden relative">
        ...
        <div className="mt-12 pt-12 border-t border-slate-200 dark:border-white/5 flex flex-wrap gap-4 relative z-10">
          <button 
            ...
            className="bg-violet-600 text-white font-black px-12 py-6 rounded-xl hover:bg-violet-700 hover:scale-105 active:scale-95 transition-all shadow-xl text-2xl uppercase italic"
          >
            {t.ctaPrimary}
          </button>
          <button 
